...
class _ClientTypesPageState extends State<ClientTypesPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<ClientTypeStore>(
          builder: (context, ClientTypeStore types, Widget? child) =>
              ListView.builder(
                itemCount: types.clientTypes.length,
                itemBuilder: (context, index) {
                  return Dismissible(
                    key: UniqueKey(),
                    background: Container(color: Colors.red),
                    child: ListTile(
                      leading: Icon(types.clientTypes[index].icon),
                      title: Text(types.clientTypes[index].name),
                      iconColor: Colors.deepOrange,
                    ),
                    onDismissed: (direction) {
                      types.removeClientType(index);
                    },
                  );
                },
              )),
    ...
    showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            actions: [
              Consumer<ClientTypeStore>(
                builder: (context, ClientTypeStore types, child) => TextButton(
